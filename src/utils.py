...
class utils():
    ...
    def get_username():
        if nameMode == "CUSTOM":
            return customName
        elif nameMode == "PREFIX":
            start = f"{prefix} | "
            amount = max(32 - len(start), 0)
            return f"{start}{utils.random_string(amount)}"
        elif nameMode == "SUFFIX":
            end = f"| {prefix}"
            amount = max(32 - len(end), 0)
            return f"{utils.random_string(amount)}{end}"
        elif nameMode == "EMOJI":
            return utils.random_emojis(rndLength)
        elif nameMode == "RANDOM":
            return utils.random_string(rndLength)
        elif nameMode == "FILE":
